...
void printprocstks(int priority){
	...
	int i = 0;	//process ID iterator
	while(i < NPROC){  //if process count is less than number of processes
		proc = &proctab[i];
		if((proc->pstate != PRFREE) && (proc->pprio > priority)){	//only if process priority is greater than input variable do we print values
			...
			if(proc->pstate == PRCURR){
				...
				esp = (unsigned long *)proc->pesp;	//else you can just get the stack pointer from the pesp variable
			}
