...
void printsegaddress(void){
  ...
  kprintf("After: etext[0x%08x]=0x%08x, edata[0x%08x]=0x%08x, ebss[0x%08x]=0x%08x", &etext+1, *(&etext+1), &edata+1, *(&edata+1), &end+1, *(&end+1)); //next segment
}
