...
#define schedprio(x)	(x->pinh!=0?x->pinh:x->pprio)
...
struct	pentry	{
	...
	int	pwaitret;
	int pinh;
	int plocksheld[50];		/* locks acquired by the process	*/
	int lockid;
	int plockused[50];		/* locks used in the past: if set, shouldn't be reset during the process' execution	*/
};
