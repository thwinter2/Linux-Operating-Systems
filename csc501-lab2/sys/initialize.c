...
*/
  ...
  for(h; h < 4; h++){
    ...
    frm_tab[frame_num].fr_pid = NULLPROC;
    page_table_entry = (frame_num + FRAME0) * NBPG;
    ...
    for(k; k < NFRAMES; k++){
			...
			page_table_entry->pt_base = (h*FRAME0) + k;
			page_table_entry++;
    }
  }
  modify_page_directory(NULLPROC);
 /* if(get_frm(&frame_num2) != OK){
    return SYSERR;
  }
  int m = 0;
  proctab[NULLPROC].pdbr=(frame_num2 + FRAME0) * NBPG;

  frm_tab[frame_num2].fr_status = FRM_MAPPED;
  frm_tab[frame_num2].fr_type = FR_DIR;
  frm_tab[frame_num2].fr_pid = NULLPROC;
  frm_tab[frame_num2].fr_vpno = 0;
	frm_tab[frame_num2].fr_refcnt = 4;
  proctab[NULLPROC].pdbr=(frame_num2 + FRAME0) * NBPG;
  page_directory_entry = proctab[NULLPROC].pdbr;

  for(m; m < NFRAMES; m++){
    //pd_t *page_directory_entry = (FRAME0 + frame_num2) * NBPG + sizeof(pd_t)*m;
    page_directory_entry[m].pd_pres = 0;
    page_directory_entry[m].pd_base = 0;
    page_directory_entry[m].pd_write = 1;
    page_directory_entry[m].pd_avail = 0;
    page_directory_entry[m].pd_user = 0;
    page_directory_entry[m].pd_pcd = 0;
		page_directory_entry[m].pd_pwt = 0;
    page_directory_entry[m].pd_mbz = 0;
		page_directory_entry[m].pd_acc = 1;
		page_directory_entry[m].pd_global = 0;
    if(m < 4){
      page_directory_entry[m].pd_pres = 1;
      page_directory_entry[m].pd_base = FRAME0 + m;
    }
  }*/

  /*if(page_replace_policy == SC){
    //head = (struct frame_node*)getmem(sizeof(struct frame_node));
    head->frame_num = -1;
    head->next = head;
  }
